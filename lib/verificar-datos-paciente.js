 ...
 *
 * @param {Array<{id: string, nombre: string, telefono: string|null, score: number}>} rows
 * @param {string} fromNumber
 * @returns {object}
 */
export function applyMatchDecision(rows, fromNumber) {
  ...
  if (rows.length > 1) {
    return {
      match: 'multiple',
      candidates: rows.map((r) => ({ id: r.id, nombre: r.nombre, score: r.score })),
    }
  }
 ...
 *
 * @param {{ nombre_completo: string, fecha_cita: string, fecha_nacimiento: string, from_number: string }} args
 * @param {object} supabase  Supabase client (service-role)
 * @returns {Promise<object>}
 */
export async function verificarDatosPaciente(args, supabase) {
  const { nombre_completo, fecha_cita, from_number } = args

  try {
    const { data: rows, error } = await supabase.rpc('verificar_paciente_match', {
    ...
    return applyMatchDecision(rows ?? [], from_number)
  } catch (err) {
