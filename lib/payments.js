...
export async function uploadComprobante(senderNumber, wamid, imageUrl) {
  ...
  try {
    ...
    // 4. Get a SIGNED URL valid for 24h.
    //    The `comprobantes` bucket is private — `getPublicUrl()` returns a URL
    //    that 400s on fetch when public=false. 24h matches the TTL of
    //    `pending_kelly_actions(action_type='aprobar_pago')`, so the URL is
    //    alive for the whole approval window (OCR call, Telegram preview to
    //    Kelly, callback handler). Discovered 2026-05-26: OCR was failing
    //    with `image_http_400` because the public URL was unreachable.
