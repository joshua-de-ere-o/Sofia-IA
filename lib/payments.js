...
export async function processPaymentImage(senderNumber, imageUrl) {
  ...
  try {
    ...
    // Buscar cita pendiente_pago (incluye monto_adelanto calculado por el agente)
    ...
    // El monto viene precalculado por el agente al agendar la cita (plan + zona)
    const monto = cita.monto_adelanto || 0;

    // 6. Registrar en la tabla `pagos`
    ...
    // 7. Notificar a Kelly por Telegram
